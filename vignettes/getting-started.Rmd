...
Below, is a true quickstart to simulate data, sample Monte Carlo trees based on that data, and finally compute the log likelihood.
...
2. Accounting for under-reporting (Multiple outside transmission model)
3. Sampling from $\mathcal{T}_n$


**Simulations**

1. [Simulations for the base model: binary covariates](not-built-vignettes/base-simulations.html)
2. [Simulations for the base model: general covariates]()
2. Simulations for the multiple outside transmission model
3. Sampling enough MC trees

**Tuberculosis in Maryland**

1. The data
2. Running our models on the data
...
beta1 <- 1.5
M <- 100
data <- simulate_general_outbreak(K = M,
...
$$
and the total likelihood of a single cluster over all trees of size $n$ is
$$
\mathcal{L}( \beta; C) = \sum_{T \in \mathcal{T}_n} L(\beta;T).
$$

We instead maximize the **approximate** likelihood, which is obtained from sampling transmission trees from $\mathcal{T}_n$.  That is, we approximate $\frac{\mathcal{L}(C, \beta)}{|\mathcal{T}_n|}$ averaging the likelihood from Monte Carlo sampled trees $T_1, \dots, T_K$,
...
### How to get Monte Carlo (MC) sampled transmission trees

If we want to sample $B$ MC transmission trees for each cluster in our data, we use the function `sample_mc_trees`.  Here, `B` is the number of MC samples.  Typically this number is $>5000$, but we use 10 below for demonstrative purposes.
...
mc_trees %>% head(., 4) %>%
  kable(type = "html") %>%
   kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
The MC samples are linked to the original cluster by the variable `orig_id`.
