...
n_seq <- c(5, 10, 15, 20, 25)
list_df <- vector(mode = "list",
                  length = length(B_seq) *
                      length(prop_pos_seq) *
                      length(n_seq))
list_index <- 0
for(kk in 1:length(n_seq)){
    n <- n_seq[kk]
    print(paste("n is", n))
    prop_pos <- .5
    ...
    for(ii in 1:length(B_seq)){
        ...
        for(jj in 1:length(prop_pos_seq)){
            ...
            sampled_loglike_df$n <- n
            list_df[[list_index]] <- sampled_loglike_df
        }
    }
}
...
## Summarize
summary_df <- plot_df %>%
    group_by(B, prop_pos, n) %>%
    summarize(mean = mean(like),
              log_mean = log(mean(like)),
              sd = sd(like),
              lower = quantile(like, .025),
              median = quantile(like, .5),
              upper = quantile(like, .975)) %>%
    arrange(n, prop_pos) %>% select(n, prop_pos, B, log_mean)


summary_df %>% filter(prop_pos == .25,
                      B > 500) %>%
    kable(format = "latex",
                        booktabs = TRUE,
          linesep = "", digits = 2,
          caption = "Simulations to show how stable the log mean likelihood is for different transmission tree sizes, which indicates whether our sampled tree likelihood estimate is a good approximation of the true likelihood.",
          label = "sim1-binary",
          col.names = c("Size", "\\% Pos. Smear",
                        "# Trees", "log $\\left(\\hat{L}_K(n)\\right )$"),
          escape = FALSE) %>%
