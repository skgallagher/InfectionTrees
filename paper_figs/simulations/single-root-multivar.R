...
best_params_mat <- readRDS("best_params.RDS")
...
apply(best_params_mat, 2, sd)

mat <- rbind(colMeans(best_params_mat),
                    c(-2, 0.1, 1.5, 0.2)
             )
rownames(mat) <- c("Est.", "Actual")

mat %>% kable(format = "latex",
                       caption = "Average parameter estimates for a 1,000 simulations of a coin flipping process with 3 covariates.",
                       label = "sim-results-many-clusters",
              align = "c", booktabs = TRUE, digits = 2,
              col.names = c("$\\beta_0$",
                            "$\\beta_1$",
                            "$\\beta_2$",
                            "$\\beta_3$"), escape = FALSE)
