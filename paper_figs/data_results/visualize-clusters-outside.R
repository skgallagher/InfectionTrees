...
feature_df <- joined_data3 %>%
    distinct_at(vars(all_of(covariate_names)), .keep_all = TRUE) %>%
    rename(index_person = feature_id) %>%
...
n <- length(rank)
theta <- -2 * pi / n * rank +   pi /2 + pi * 2 / n
feature_df$x <- cos(theta)
feature_df$y <- sin(theta)
feature_df$theta <- theta
...
ggplot(final_df,
  ...
  #  geom_hline(yintercept = 0, linetype  = "dashed") + 
    ...
    geom_label(aes(label = lapply(paste0("$\\hat{p}$ = ",
                   ...
                   parse = TRUE,
               nudge_y = .25 * sin(final_df$theta),
               nudge_x = .4 * cos(final_df$theta),
               size = 8) +
    geom_point(aes(x = x_from + 0.0 * cos(theta),
                   y = y_from + 0.0 * sin(theta),
                   fill = factor(hiv_neg_pos)),
               size = 6, shape = 22) +
